...
public sealed class WindowsPrintSpooler : IPrinterSpooler
{
    ...
    {
        var printSettings = string.IsNullOrWhiteSpace(_options.PrintSettings) ? "fit" : _options.PrintSettings.Trim();
        var args = $"-silent -print-to \"{printerName}\" -print-settings \"{printSettings}\" \"{pdfPath}\"";
        ...
        try
        {
            _logger.LogInformation("Enviando PDF a impresión. Printer={Printer} Settings={Settings}", printerName, printSettings);
